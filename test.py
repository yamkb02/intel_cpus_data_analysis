...
# Create a pie chart with a dark theme
...
# Create a bar chart of core counts with a dark theme
...
# Create a scatter plot of ClockSpeedMax vs Price with a dark theme
...
# Create a scatter plot of CoreCount vs Price with a dark theme
...
# Create a box plot to show the distribution of clock speed max for each lithography group using Plotly with a dark theme
...
# Create a regression plot to show the relationship between price and core count using Plotly with a dark theme
...
page_content = dbc.Container(
    [
        dbc.Row(
            [
                dbc.Col(html.H1("Intel CPUs", className="display-5 text-center mb-2"), md=6),
                dbc.Col(
                    dbc.Container(
                        [
                            dbc.Button("Home", href="/", color="primary", className="mr-1", style={"background-color": "#1d2439", "border-color": "#1d2439", "font-size": "1.5rem", "color": "white", "transition": "background-color 0.2s, border-color 0.2s"}),
                            dbc.Button("About", href="/about", color="primary", className="mr-1", style={"background-color": "#1d2439", "border-color": "#1d2439", "font-size": "1.5rem", "color": "white", "transition": "background-color 0.2s, border-color 0.2s"}),
                        ],
        ...
        dbc.Row(
            dbc.Col(dbc.Card(dcc.Graph(figure=fig5), className="p-3 shadow-lg", style={"background-color": "#2e3858"}), md=12),
            className="mb-4",
        ),
        dbc.Row(
            dbc.Col(dbc.Card(dcc.Graph(figure=fig6), className="p-3 shadow-lg", style={"background-color": "#2e3858"}), md=12),
            className="mb-4",
        ),
        dbc.Row(
            dbc.Col(dbc.Card(dcc.Graph(figure=fig7), className="p-3 shadow-lg", style={"background-color": "#2e3858"}), md=12),
            className="mb-4",
        ),
        dbc.Row(
            dbc.Col(dbc.Card(dcc.Graph(figure=fig8), className="p-3 shadow-lg", style={"background-color": "#2e3858"}), md=12),
            className="mb-4",
        ),
    ],
    fluid=True,
...
about_page = dbc.Container(
    [
        ...
        html.P("This is a sample about page. You can edit this text to provide more information about the dashboard."),
    ],
    fluid=True,
    className="p-0"  # Set className to "p-0" to remove the padding on the left and right sides
)
...
app.layout = dbc.Container(
    ...
    fluid=True,
    style={"background-color": "#1d2439"}  # Set the background color of the overall webpage
)
